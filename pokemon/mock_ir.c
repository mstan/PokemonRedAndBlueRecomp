 ...
 * and the game_dispatch_override that intercepts each Gen 2 cart's
 * ExchangeMysteryGiftData entry point.
 *
...
#include "gbrt.h"
#include "game_extras.h"  /* game_dispatch_override hook signature */
...
/* Agnostic dispatch hook (see runtime/include/game_extras.h). The generated
 * dispatch loop calls this for each address it routes through the trampoline;
 * returning 1 means "handled, pc/state already advanced — skip normal
 * dispatch". We intercept the Gen 2 ExchangeMysteryGiftData entry and apply
 * the queued gift. Everything else (and every non-Gen-2 cart, e.g. Red/Blue)
 * returns 0 and dispatches normally.
 *
 * NOTE: this replaces an old GB-Recomp `gb_dispatch` override that interpreted
 * everything — incompatible with static recompilation (where gb_dispatch is
 * generated and is the sole definition). Full Gen 2 Mystery Gift validation
 * needs a Gen 2 cart + Ghidra check of the call path; dormant for Gen 1. */
